"""
NephroAI — CKD Prediction Streamlit App — FIXED VERSION
Run: streamlit run app.py

Fixes applied:
  1. Double artifact unpack removed — single unpack after None check
  2. `su` and `pot` exposed as sidebar inputs (no longer silently hardcoded)
  3. local_explain signature aligned with fixed pipeline (no background args)
  4. model_results leaderboard now displays full metrics (Precision, Recall, F1)
"""
...
# ─── FIX 1: Single artifact unpack block ─────────────────────────────────────
...
st.sidebar.markdown("*Leave blank / use defaults to auto-impute from population mean*")
...
# FIX 2: pot (Potassium) now exposed — was silently hardcoded to 4.5
pot  = si("Potassium (mEq/L)",          2.5,   7.5,  0.1,   4.5,  "Normal: 3.5–5.0",       "pot")
...
# FIX 2: su (Sugar) now exposed — was silently hardcoded to 0
su      = st.sidebar.slider("Sugar (0–5 scale)", 0, 5, 0)
...
input_data = {
    ...
    'sg':    sg,
    'al':    al,
    'su':    su,           # FIX 2: from sidebar, not hardcoded

    # Categorical as strings — required for encoder lookup
    'rbc':   'normal',
    'pc':    'normal',
    ...
    'sc':    sc,
    'sod':   sod,
    'pot':   pot,          # FIX 2: from sidebar, not hardcoded
...
def local_explain(model, sample, features):
    ...
    Perturbs each feature to its scaled mean (0) and measures prediction delta.
    """
...
if predict_btn:
    ...
    with col1:
        st.markdown(f"""<div class='metric-card'>
            <div class='{"risk-high" if isCKD else "risk-low"}'>
                {'CKD Detected' if isCKD else 'No CKD'}
            </div>
            <div style='color:#8899bb;margin-top:8px'>Prediction Result</div>
        </div>""", unsafe_allow_html=True)

    with col2:
        prob_color = "#f43f5e" if prob > 0.7 else "#f59e0b" if prob > 0.4 else "#10b981"
        st.markdown(f"""<div class='metric-card'>
            <div style='font-size:2.5rem;font-weight:bold;color:{prob_color}'>{pct:.1f}%</div>
            <div style='color:#8899bb;margin-top:8px'>CKD Probability</div>
        </div>""", unsafe_allow_html=True)

    with col3:
        risk_lv = "HIGH" if prob >= 0.7 else "MODERATE" if prob >= 0.4 else "LOW"
        risk_colors = {"HIGH": "#f43f5e", "MODERATE": "#f59e0b", "LOW": "#10b981"}
        st.markdown(f"""<div class='metric-card'>
            <div style='font-size:2.5rem;font-weight:bold;color:{risk_colors[risk_lv]}'>{risk_lv}</div>
    ...
    with left:
        ...
        ax.barh(names[::-1], vals[::-1], color=colors_bar[::-1], height=0.6)
    ...
    with right:
        ...
        gcols     = [
            '#f43f5e' if global_imps.get(n, 0) > 0.08
            else '#3b82f6' if global_imps.get(n, 0) > 0.04
            else '#10b981'
            for n in gnames
        ]
        ...
        ax2.legend(handles=[red_p, blue_p, green_p],
                   facecolor='#0a0f1e', labelcolor='#8899bb', fontsize=9)
        plt.tight_layout()
        st.pyplot(fig2)
        plt.close()
    ...
    feat_labels = {
        'hemo': 'Hemoglobin',      'rbcc': 'RBC Count',     'pcv':   'PCV',
        'sc':   'Serum Creatinine','bu':   'Blood Urea',     'al':    'Albumin',
        'appet':'Appetite',        'htn':  'Hypertension',   'sod':   'Sodium',
        'bp':   'Blood Pressure',  'wbcc': 'WBC Count',      'pcc':   'Pus Cell Clumps',
        'bgr':  'Blood Glucose',   'sg':   'Specific Gravity','age':  'Age',
        'pot':  'Potassium',       'su':   'Sugar'
    }
...
**Electrolytes:** Sodium {sod:.0f} mEq/L — {'⚠ Hyponatremia' if sod < 135 else '✓ Normal'}. Potassium {pot:.1f} mEq/L — {'⚠ Abnormal' if pot < 3.5 or pot > 5.0 else '✓ Normal'}.
...
> ⚠ *AI-generated summary — must be reviewed by a qualified clinician.*
    ...
    # ── FIX 4: Model Leaderboard now shows full metrics ───────────────────────
    st.markdown("## 🏆 Model Performance Leaderboard")
    st.markdown("Evaluated on held-out test set (80/20 stratified split) after SMOTE balancing")

    df_results = pd.DataFrame(results_dict)

    # Sort by AUC if present, else Accuracy
    sort_col = "AUC" if "AUC" in df_results.columns else "Accuracy"
    ...
    ax.set_title('SHAP-Equivalent Feature Importance — All Selected Features',
                 color='#e8edf8', pad=12)
    for spine in ax.spines.values():
        spine.set_edgecolor('#1e2d4a')
